...
SAZ 캡처로 확정. **2단계**: 임시 업로드 → 메시지의 `attachmentList` 로 첨부.
채팅은 MD/HTML 을 인라인 렌더하지 않는다 — 수신자가 **다운로드**하는 파일이다.
...
- `attachmentId` = `-1` (신규), `fileStatus` = `"UPLOADED"`, `fileType` 는 빈
  문자열로 관측됨. `sender` 는 로그인 시 해석된 봇 신원.
- 캡처엔 `expiredAt` 도 있었으나 업로드 응답엔 없어 출처 불명 — SDK 는
  보내지 않는다(서버가 부여하는 것으로 추정, **라이브 검증 필요 항목**).

> SDK: `BotClient.upload_attachment(path) -> data` + `send_message(room,
> content, attachments=[data])`, 단축 `send_file(room, path, content="")`
> (`DaouBot.send_file` async). 계약은 SAZ 기반·**라이브 미검증**.
