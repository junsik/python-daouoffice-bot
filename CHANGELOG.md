...
- 설정 파일은 `~/.daoubot/profile.json` 단일 파일 — 홈 디렉터리 기준이라 실행 cwd 와 무관(`~/.aws`/`~/.docker` 류 관례); 어느 디렉터리·예제에서 실행하든 한 번의 `daoubot login` 으로 동작 (SDK는 `.env`/dotenv를 읽지 않음). `profile.example.json`이 전체 필드 형태를 보여줌 (시크릿 필드는 빈 값); `--config <path>` (서브커맨드 이후)로 다중 봇/테넌트 호스트에서 위치 변경. `load_profile`/`save_profile`/`load_settings`가 명시적 경로 인자를 받음.
