...
- `BotEngine` 디스패치가 **방 간 병렬 / 방 내 직렬** 로 바뀜. 이전에는 `_poll_once` 가 방을 순차로 `await` 해 한 방의 느린 핸들러(예: 수십 초 걸리는 LLM 호출)가 그 폴링 사이클 전체 — 다른 방과 다음 폴링 — 를 멈췄음. 이제 방마다 핸들러를 `asyncio.Task` 로 띄워 동시에 처리하되, **같은 방은 동시에 두 번 처리하지 않음**: 이전 핸들러가 아직 돌고 있는 방은 이번 사이클에 건너뛰고(커서 미전진 → 메시지 pending 유지) 다음 폴링에서 다시 잡음.
- **at-least-once / 방 내 순서 보장은 그대로**. 방은 핸들러 성공 후에만 커서 전진 + `mark_read`; 핸들러 진행 중인 방은 스킵될 뿐 메시지가 유실되거나 순서가 뒤집히지 않음. 종료 시 진행 중인 방 핸들러는 취소·드레인되며, 취소된 핸들러의 메시지는 다음 실행에서 재배달됨.
- 공개 API·핸들러 계약 변경 없음 (`on_message` 시그니처 동일). 호출측 코드 수정 불필요 — 동작만 개선됨. 핸들러가 공유 상태를 만지면 이제 방 간 동시 호출될 수 있음에 유의 (방별 직렬은 유지).
