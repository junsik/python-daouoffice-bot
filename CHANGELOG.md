...
- Claude Code skill `.claude/skills/daouoffice-bot/` (SKILL.md + reference.md + scaffold.py): a **design guide**, not a template menu — the agent elicits requirements, maps them to primitives via a decision matrix, and assembles the bot under the SDK's invariants (no BotFather/ webhooks/inline; polling, allowlist, idempotency, env/profile config). `scaffold.py` emits only correct boilerplate (UTF-8 safe on cp949 Windows); the design is the agent's, from the user's actual needs.
