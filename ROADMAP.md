...
- **WebSocket / STOMP 실시간** (`GET /ws/pc`). 캡처에서 엔드포인트 확인했지만 흐름은 검증되지 않음, 따라서 **구현되지 않음** 및 **일정 없음**. 프로모션 필요: 라이브 테넌트에서clean 검증된 캡처, 테스트된 클라이언트, 폴링의 전달 보장과 동등성.即便如此, **폴링이 지원된 기본값으로 유지**; WS는 opt-in. 검증 전까지 추측성 코드는 들어오지 않음.
